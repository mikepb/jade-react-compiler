    ...
    if (b.isIdentifier(call.callee)) {
      switch (call.callee.name) {
        case 'ǃDOM＿':
          ...
          // visit arguments
          call.arguments = this.nullWithNext(
            call.arguments.map(this.visit).filter(notEmpty));
        ...
        case 'ǃtext＿':
          ...
          return this.concat(call.arguments.map(this.visit), '\n');
        case 'ǃunescape＿':
          if (!call.arguments.length) return;
          call.callee = this.domCall('text');
          call.arguments = [
            this.escape(this.concat(
              call.arguments.map(this.visit), '\n'))];
          break;
        default:
  ...
  nullWithNext: function (body) {
    for (var j = 0; j < body.length - 1; j++) {
      ...
      if (b.isSequenceExpression(next)) {
        node.expressions = exprs.concat(next.expressions);
      } else {
        exprs.push(next);
      }
      body.splice(j + 1, 1);

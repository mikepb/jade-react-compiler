      ...
      switch (call.callee.name) {
        case 'ǃDOM＿':
          ...
          args.forEach(function (it) {
            ...
            if (!b.isCallExpression(it) ||
              !b.isMemberExpression(it = it.callee) ||
              !b.isIdentifier(it = it.property) ||
              it.name !== 'forEach') return;
            it.name = 'map';
          });
          // visit arguments
          args = this.nullWithNext(args.map(this.visit).filter(notEmpty));

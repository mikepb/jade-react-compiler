...
Rectifier.prototype = {
  ...
  rectify: function () {
    ...
    // console.log(JSON.stringify(this.program, null, '  '));
    return this.program;
  },
  ...
  visit: function (node) {
    // this.indent++;
    // console.log(new Array(this.indent).join('  ') + node.type);
  ...
  visitProgram: function (program) {
      ...
      exprs = [
        {
          ...
          arguments: this.nullWithNext(exprs)
        }
      ];
    }
    ...
    if (this.funs.length) {
      [].unshift.apply(statements, this.funs);
    }
  ...
  visitIfStatement: function (ifs) {
    return {
      ...
      consequent: this.visit(ifs.consequent),
      alternate: ifs.alternate ? this.visit(ifs.alternate) : this.nullLiteral
    };
  },
  ...
  collapseStatements: function (body) {
    for (var i = 0; i < body.length; i++) {
        ...
        next && next.type === 'BlockStatement')
      {
        ...
        node.callee = this.makeDOMCall(node.arguments.shift());
      ...
      // concatenate sequential texts
        ...
        if (this.domAt == null || this.domAt === this.domDepth) {
          node = node.expression;
          node.callee = this.makeDOMCall('text');
  ...
  makeSequence: function (nodes) {
    ...
    if (nodes.length === 0) return this.nullLiteral;
